...
const BoxPlot: React.FC<BoxPlotProps> = ({ data }) => {
  ...
  const calculateOverallStats = () => {
    const allValues = data.flatMap(group => group.categories.flatMap(category => category.values));
    const overallMean = d3.mean(allValues) || 0;
    const variance = d3.mean(allValues.map(v => Math.pow(v - overallMean, 2))) || 0;
    const stdDevAcrossSamples = Math.sqrt(variance);

    const groupMeans = data.map(group => d3.mean(group.categories.flatMap(category => category.values)) || 0);
    const varianceBetweenSamples = d3.mean(groupMeans.map(v => Math.pow(v - overallMean, 2))) || 0;
    const stdDevBetweenSamples = Math.sqrt(variance);

    const confidenceInterval = [overallMean - 1.96 * stdDevAcrossSamples, overallMean + 1.96 * stdDevAcrossSamples];
  ...
  useEffect(() => {
    ...
    const categories = Array.from(new Set(data.flatMap(d => d.categories.map(c => c.category))));
    ...
    const maxY = d3.max(data.flatMap(groupData => groupData.categories.flatMap(categoryData => {
    ...
    gridGroup.selectAll('.horizontal-grid-line')
      .data(y.ticks())
      .enter().append('line')
    ...
    gridGroup.selectAll('.vertical-grid-line')
      ...
      .attr('x1', (d: number) => x1(d) + x1.bandwidth() / 2)
      .attr('x2', (d: number) => x1(d) + x1.bandwidth() / 2)
      .attr('y1', 0)
      .attr('y2', height)
      .style('stroke', '#ccc')
      .style('stroke-opacity', 0.5);
...
if(groups.length < 2){
  data.forEach((groupData) => {
    ...
    if (!groupData.intervals) {
      const allValues = groupData.categories.flatMap((category) => category.values).sort(d3.ascending);
      ...
      groupData.intervals = [
        { type: 'confidence interval', percentage: '95%', values: confidenceInterval },
        { type: 'tolerance interval', percentage: '95%', values: toleranceInterval },
      ];
    }
...
data.forEach(groupData => {
  ...
  const calculateBoxPlotStats = (values: number[]): { min: number, q1: number, median: number, q3: number, max: number, mean: number, stdDev: number, outliers: number[] } => {
    const sortedValues = values.sort(d3.ascending);
    const q1 = d3.quantile(sortedValues, 0.25)!;
    const median = d3.quantile(sortedValues, 0.5)!;
    const q3 = d3.quantile(sortedValues, 0.75)!;
    const iqr = q3 - q1;
    const min = Math.max(...sortedValues.filter(v => v <= q1 + 1.5 * iqr), Math.min(...sortedValues));
    const max = Math.min(...sortedValues.filter(v => v >= q3 - 1.5 * iqr), Math.max(...sortedValues));
    const mean = d3.mean(values)!;
    const variance = d3.mean(values.map(v => Math.pow(v - mean, 2)))!;
    const stdDev = Math.sqrt(variance);

    const outliers = sortedValues.filter(v => v < max - stdDev || v > min + stdDev);
    ...
    <table>
      <tbody>
        <tr>
          ...
          <td>[{stats.confidenceInterval[0].toFixed(2)}, {stats.confidenceInterval[1].toFixed(2)}]</td>
        </tr>
        <tr>
          <td>Standard deviation between samples</td>
          <td>{stats.stdDevBetweenSamples.toFixed(2)}</td>
          <td>95% Tolerance interval</td>
          <td>[{stats.toleranceInterval[0].toFixed(2)}, {stats.toleranceInterval[1].toFixed(2)}]</td>
        </tr>
        <tr>
