...
const DensityPlotter: React.FC<{
  ...
  useEffect(() => {
    ...
    const kdeDataAll: [number, number][][] = datasets.map((data) => kde(data));
    const flattenedData: [number, number][] = kdeDataAll.flat();
    const yMax = d3.max(flattenedData, (d:any) => d[1]) || 1;
    ...
    const gSelection: Selection<SVGGElement, unknown, null, undefined> = g
      .append("g")
      .call(yAxis)
      ...
      .call((g: Selection<SVGGElement, unknown, null, undefined>) =>
        g.selectAll(".tick text").attr("fill", "#000")
      )
      .append("text")
    ...
    datasets.forEach((data, i) => {
      ...
      g.selectAll(`.dot${i}`)
        ...
        .attr("cy", (d: { x: number; y: number }) => yKDE(d.y))
        .attr("stroke", "black")
        .attr("r", 3)
        .attr("fill", color)
        .on(
