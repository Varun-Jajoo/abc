...
const GlutenDetectionGraph: React.FC<Props> = ({ data, xAxisLabel, yAxisLabel, drop=false, log }) => {
    ...
    const interpolateLimits = (curve: DataPoint[], upperLimit: DataPoint[], lowerLimit: DataPoint[]) => {
    // Combine all x values from curve, upperLimit, and lowerLimit
    let allXValues = new Set([...curve.map(d => d.x), ...upperLimit.map(d => d.x), ...lowerLimit.map(d => d.x)]);
    ...
    allXValues.forEach(xValue => {
        ...
        if (!upperLimit.some(point => point.x === xValue)) {
            let lowerPoint = upperLimit.filter(point => point.x < xValue).slice(-1)[0];
        ...
        if (!lowerLimit.some(point => point.x === xValue)) {
            let lowerPoint = lowerLimit.filter(point => point.x < xValue).slice(-1)[0];
    ...
    useEffect(() => {
        ...
        const x = log ? d3.scaleLog().base(log).domain([1, xMax]).range([margin.left, width - margin.right-10])
        : d3.scaleLinear().domain([0, xMax]).range([margin.left, width - margin.right-10]);
...
const yAxis = d3.axisLeft(y);
        ...
        const drawArea = (lowerLimit: DataPoint[], upperLimit: DataPoint[]) => {
            ...
            if (!lowerLimit || !upperLimit || lowerLimit.length === 0 || upperLimit.length === 0) {
                console.error('Lower limit or upper limit arrays are empty or undefined.');
                return;
            }
        ...
        const drawLabDetectionRates = (group: GroupData, color: string) => {
            ...
            svg.selectAll(`.lab-detection-${group.name}`)
        ...
        data.forEach(group => {
            ...
            const { interpolatedUpper, interpolatedLower } = interpolateLimits(group.lpodCurve, group.upperLimit, group.lowerLimit);
        ...
        const legend = svg.append('g')
            .attr('class', 'legend')
            .attr('transform', `translate(${width - margin.right - 300},${height - 185})`);
    ...
    const findIntersectionX = (curveData: DataPoint[], yValue: number) => {
        for (let i = 1; i < curveData.length; i++) {
            if (curveData[i - 1].y <= yValue && curveData[i].y >= yValue) {
                const slope = (curveData[i].y - curveData[i - 1].y) / (curveData[i].x - curveData[i - 1].x);
                const xValue = curveData[i - 1].x + (yValue - curveData[i - 1].y) / slope;
                return xValue;
            }
        }
        return null;
